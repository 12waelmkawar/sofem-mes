...
const OP_STATUT_LABELS: Record<string, string> = {
  PENDING: 'En attente',
  IN_PROGRESS: 'En cours',
  COMPLETED: 'Terminee',
};
const OP_STATUT_COLORS: Record<string, 'red' | 'green' | 'blue' | 'orange' | 'muted'> = {
  PENDING: 'muted',
  IN_PROGRESS: 'orange',
  COMPLETED: 'green',
};
...
export default function OrdersPage() {
  ...
  return (
    <div>
      ...
      {opsOF && <OperationsModal of={opsOF} onClose={() => setOpsOF(null)} canManage={manager} />}
    </div>
  );
}
...
function EditModal({ of, onClose }: { of: OF; onClose: () => void }) {
  return (
    <Modal open onClose={onClose} title={`Modifier ${of.numero}`} width="max-w-xl">
      <p className="text-[var(--muted)] text-sm mb-4">Edit complet: {of.produit_nom} × {of.quantite} | Statut: {STATUT_LABELS[of.statut]}</p>
      <div className="flex justify-end gap-2">
        <Button variant="secondary" onClick={onClose}>Fermer</Button>
        <Button onClick={() => { toast.info('Edition complete en Phase 8'); onClose(); }}>Enregistrer</Button>
      </div>
    </Modal>
  );
}
